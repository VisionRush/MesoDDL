...
class ConvNeXt(timm.models.convnext.ConvNeXt):
    def __init__(self,conv_pretrain=False):
        super(ConvNeXt, self).__init__(depths=(3, 3, 27, 3), dims=(128, 256, 512, 1024))
        if conv_pretrain:
...
class UpsampleConcatConv(nn.Module):
    def __init__(self):
        ...
        # 192到96的上采样，单次上采样
        self.upsamplec2 = nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1)
...
class Mesorch_512(nn.Module):
    def __init__(self, seg_pretrain_path=None, conv_pretrain=False, if_predict_label=True):
        super(Mesorch_512, self).__init__()
        ...
        self.inverse = nn.ModuleList([nn.Conv2d(128, 1, 1) for _ in range(4)]+[nn.Conv2d(64, 1, 1) for _ in range(4)])
    ...
    def forward(self, image, mask, label, *args, **kwargs):
        ...
        # pred_label=self.label_head(features).squeeze(-1)
        ...
        pred_label=gmp(pred_mask).squeeze(1).squeeze(1).squeeze(1)
        # label_loss = self.loss_fn(pred_label,label.float())
        loss = mask_loss 
        output_dict = {
            # loss for backward
